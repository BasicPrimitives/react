# Using Basic Primitives Org Chart with React State

When using basic primitives org chart with React component state, the chart automatically gets reset when state of parent component changes.
To encounter this issue, a simple solution is to use `React.memo` and render orgchart only when config changes. This will prevent resetting of org chart if state of parent component changes.
...
const Diagram = ({ config, onCursorChanged }) => {
...
const isValEqual = (prev, next) => (
  equal(prev.config, next.config)
);

const OrgDiagramV2 = React.memo(Diagram, isValEqual);
...
function App() {
  ...
  const onCursorChanged = (e, data) => {
            setState(data.context.title);
  };
